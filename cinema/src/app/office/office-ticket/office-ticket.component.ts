...
export class OfficeTicketComponent implements OnInit {
  ...
  constructor(
    ...
    private adminFilm: AdminFilmService
  ) { }
  ...
  getPlaces(): void {
    this.adminFilm.getPlaces().subscribe(data => {
      this.allPlaces = data
    })
  ...
  deleteFilm(item, i, elem): void {
    ...
    let id = this.allPlaces[i].id
    console.log(id);

    this.adminFilm.deletePlaces(id).subscribe(() => {
      this.getPlaces()
    })
